  ...
  constructor(window) {
    ...
    const ENV_ = (
      (globalThis.process && globalThis.process.env)
        ? 'Node'
        : (_ => (globalThis.window && globalThis.document)
          ? (_ => globalThis.import?.meta?.url
            ? 'ESM'
            : 'Web'
          )()
          : 'Unknown'
    ...
    class Cabra {
      constructor({ route = '', scripts = [], styles = [], htmls = [] }) {
        this.route = route;
        ...
        this.scripts = importAll(this.scripts);

        this.htmls = this.htmls.map(html => {
          if (typeof html === 'string' && (html.startsWith('http://') || html.startsWith('https://') || html.startsWith('./') || html.startsWith('../')))
            return import(html);
          else
    ...
    function sineTween(element, targetPosition, duration = 1000) {
      return new Promise((resolve, reject) => {
        const startTime = performance.now();
        const startPosition = element.getBoundingClientRect();
        const distanceX = targetPosition.x - startPosition.x;
        const distanceY = targetPosition.y - startPosition.y;
        ...
        function update() {
          ...
          element.style.transform = `translate(${startPosition.x + distanceX * sineValue}px, ${startPosition.y + distanceY * sineValue}px)`;
    ...
    async function hydrate() {
      ...
      for (let i = 0; i < entries.length; i++) {
        ...
        a: for (let j = 0; j < entryArray.length; j++) {
          ...
          // If element has data-nohyd, skip
          if (element.hasAttribute('data-nohyd')) {
            continue a;
          } else
              ...
              if (resURL.hash.startsWith('#/') || (href.startsWith('#'))) {
                ...
                element.addEventListener('click', async (e) => {
                  ...
                  for (let k = 0; k < scripts.length; k++) {
                    ...
                    let ALLOWS = !script.getAttribute('nohydrate');
      ...
      await new Promise(async (resolve, reject) => {
        g: for (let i = 0; i < Grecha.preloaded.length; i++) {
          ...
          // If element has data-nohyd, skip
          if (element.tag.hasAttribute('data-nohyd')) {
            continue g;
          }
          ...
          async function isCorsEnabled(url) {
            try {
              ...
              // An error occurred, likely due to CORS restrictions
              return false;
            }
          }
          ...
          if (!isCorsEnabled(url)) {
            console.log('horaay!!')
            fetchedElement = await fetch(url);
          } else {
            continue;
          }

          // ---
          // AT THIS POINT; fetchedElement Exists.
          // ---
    ...
    const windowMethods = {
      ...
      async import_(url = '', as = '') {
        ...
        switch (ENV_) {
          ...
          case 'Node':
            ...
            return { [as]: typeof mod == 'object' ? { ...mod } : mod };
            break;
          ...
          case 'Web':
            ...
            script.onload = () => {
              console.info('[GS Optimizer] Script loaded.')
            }
            ...
            return { [as]: typeof window[as] == 'object' ? { ...window[as] } : window[as] };
      ...
      async importAll(...urls) {
        let promises = [];
        for (let url of urls) {
          promises.push(await window.import_(url))
        }
